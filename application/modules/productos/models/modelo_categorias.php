...
class Modelo_categorias extends CI_Model {
	...
	public function listar($cantidad = false, $pagina = false, $where = false){
        ...
        if($result){
			...
			foreach($result as $resultado){
				...
				$obj->cantidad = $this->total_productos(array("ca_codigo" => $obj->codigo));
				$listado[] = $obj;
			}
			return $listado;
        }else {
	...
	private function total_productos($where = false){
		$this->db->where("pr_visible", 1);
		$this->db->where("pr_estado", 1);
		if($where) $this->db->where($where);
